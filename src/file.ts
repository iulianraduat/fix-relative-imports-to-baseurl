...
export function fixImportsInAllFiles(workspaceRoot: string, baseUrlWithTrailingSlash: string) {
  ...
  srcFolders.forEach((folder) => {
    const { countFixes, countFiles } = fixImportsInAFile(path.resolve(workspaceRoot, folder), baseUrlWithTrailingSlash);
    totalCountFixes += countFixes;
    totalCountFiles += countFiles;
  });

  const importsWere = singularOrPlural(totalCountFixes, 'import was', 'imports were');
...
function fixImportsInAFile(
  ...
  const countFixes: number = tsFiles.reduce((prev: number, filePath: string) => {
...
function getSrcFolders(): string[] {
  return vscode.workspace.getConfiguration().get('fixRelativeImportsToBaseurl.sources', ['src']);
}
...
function fixIt(filePath: string, baseUrlWithTrailingSlash: string): number {
  ...
  const { countFixes, newCode } = getFixedImports(filePath, code, baseUrlWithTrailingSlash);
